...
## Post-Deploy Client Fix (2026-07-09)

Real client feedback from Alex after reviewing the live deployed site raised two issues. Both root-caused and fixed against the live rendering, then rechecked.

| # | Client feedback (verbatim) | Root cause | Fix |
|---|---|---|---|
| 1 | "Fred's Gym on the home page, right below the initial message on the bespoke programmes, not a class timetable... There's just an entirely blank space." | `assets/css/style.css` `.intro-grid` (Home, "Bespoke programmes, not a class timetable" section) is a 2-column CSS grid (`1.1fr 1fr`). Its second column contains `.chip-row`, a `display:flex; overflow-x:auto` row whose `.chip` items use `flex: none` (unshrinkable). Grid items default to `min-width: auto`, so the column's minimum track size was forced to the chip row's full unwrapped content width. Measured live: `grid-template-columns` computed as `275.359px 1701.47px` instead of a balanced ~604px/548px split — the intended 2-column split blew out, squeezing the left text column to 275px (text wrapped tall, 924px) while the right column stayed short (108px), leaving a large blank paper-colored area filling the rest of the section beside/below the short column. Reproduced with a full-page screenshot at the section's scroll position: 100% blank cream-colored frame, matching the client's description exactly. | Added `.intro-grid > div { min-width: 0; }` in `assets/css/style.css` (with a comment explaining the grid-item min-width footgun). This lets the grid track honour the intended 1.1fr/1fr split and lets `.chip-row`'s own `overflow-x: auto` handle scrolling instead of forcing the whole column wide. |
| 2 | "I'm really unsure why you've made the qualifications a select option... Just put the qualifications as included there. Don't make someone have to click on it to see it." | `trainers.html`: all 10 trainer cards wrapped their qualifications list in `<details><summary>Qualifications</summary><ul>...</ul></details>` (a native collapsed/click-to-reveal element — the AGENTS.md-recommended pattern for FAQs, but wrongly applied here to core bio content the client wants always visible). | Replaced all 10 `<details>/<summary>` blocks in `trainers.html` with a plain always-visible `<div class="trainer-quals"><p class="quals-label">Qualifications</p><ul>...</ul></div>` — same qualification facts (verbatim from `BUILD_BRIEF.md`/source site), zero interaction required. Updated `assets/css/style.css`: replaced `.trainer-body details`/`summary` (+/− toggle icon) rules with `.trainer-quals`/`.quals-label` styling, and removed the now-dead `.trainer-grid:has(details[open]) ...` sibling-dimming rule. Also updated the Trainers page-header lede copy, which previously said "open each profile for qualifications," to "full qualifications are listed on every profile below." |

### Recheck evidence (live preview, `localhost:4226`, after fix)

| Check | Result | Evidence |
|---|---|---|
| Intro-grid column split (desktop, 1280px) | PASS | `grid-template-columns` computed as `569.891px 518.109px` (was `275.359px 1701.47px`) — correctly proportioned to 1.1fr/1fr. Section height dropped from 1129px to 759px. Full-page screenshot (1280×2000 viewport) confirms both the "Bespoke programmes, not a class timetable" text column and the "What the trainers cover" chip row render side by side with no blank gap. |
| Intro-grid layout, mobile (375px) | PASS | Screenshot confirms single-column stack (heading, both paragraphs, stat row, then "What the trainers cover" chip row with working horizontal scroll) — no blank space anywhere in the section. |
| Qualifications always visible, desktop | PASS | `document.querySelectorAll('details').length === 0` on `trainers.html`; `document.querySelectorAll('.trainer-quals').length === 10`. Screenshot confirms "Qualifications" heading + full bullet list rendered directly under each trainer's bio paragraph with no toggle/summary control present. |
| Qualifications always visible, mobile (375px) | PASS | Screenshot confirms first card's "Qualifications" list (MSc Osteopathy, BSc Kinesiology, Personal Trainer (CSEP) 14 years, Nutritional Science & Acupuncture) fully visible with zero interaction, immediately below the bio text. |
| Qualification facts preserved | PASS | Diffed all 10 `<ul>` qualification lists against the pre-fix HTML and `BUILD_BRIEF.md` — identical content, only the wrapping element changed (`<details><summary>` → `<div><p>`). No facts added or dropped. |
| Contrast audit (regression check) | PASS | index.html: 71 text nodes, 0 violations, 0 needsManualCheck (only the 5 pre-existing documented hero-on-photo false positives, unchanged). trainers.html: 104 text nodes, 0 violations, 0 needsManualCheck — identical totals to the original REVIEW pass, confirming the qualifications markup change didn't alter contrast anywhere. |
| Upscale/broken-image audit (regression check, desktop 1280px) | PASS | index.html: 6/6 images, 0 violations/broken/mismatches. trainers.html: 10/10 images, 0 violations/broken/mismatches. Neither fix touched image markup, confirmed no regression. |

## Verdict
PASS — both client-reported issues root-caused, fixed, and independently rechecked live at desktop and mobile widths with zero regressions on contrast or image audits. Cleared for DEPLOY.
